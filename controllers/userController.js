...
const  registerUser = asyncHandler(async (req, res) => {
  const { name, matrucule, email, password } = req.body;

  // Validation
  if (!name || !matrucule || !email || !password) {
    res.status(400);
...
const sendLoginCode = asyncHandler(async (req, res) => {
  ...
  // Send Login Code
  const subject = "Login Access Code - INDUS";
  const send_to = email;
  const sent_from = process.env.EMAIL_USER;
  const reply_to = "abdelkarim.ezzar@polytechnicien.tn";
...
const sendVerificationEmail = asyncHandler(async (req, res) => {
  ...
  // Send Email
  const subject = "Verify Your Account - INDUS";
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  // Send Email
  const subject = "Password Reset Request - INDUS";
  const send_to = user.email;
  const sent_from = process.env.EMAIL_USER;
  const reply_to = "abdelkarim.ezzar@polytechnicien.tn";
